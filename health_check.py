...
IMAGE_SIZE_THRESHOLD_KB = 50  # Minimum acceptable image size in KB
...
def send_health_check(component_name, status, value=None, remarks=""):
    ...
    # Print the health check data to terminal
    print(f"[{payload['timestamp']}] [{component_name}] STATUS: {status} | VALUE: {value} | REMARKS: {remarks}")

    try:
        response = requests.post("http://localhost:8000/api/health", json=payload)
        response.raise_for_status()
    except requests.RequestException as e:
        print(f"[{component_name}] Failed to send health check: {e}")
